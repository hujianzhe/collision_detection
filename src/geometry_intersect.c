...
int Segment_Intersect_ConvexMesh(const CCTNum_t ls[2][3], const GeometryMesh_t* mesh) {
	...
	for (i = 0; i < mesh->polygons_cnt; ++i) {
		if (Segment_Intersect_Polygon(ls, mesh->polygons + i)) {
			return 1;
		}
	}
	return 0;
...
int Polygon_Intersect_Polygon(const GeometryPolygon_t* polygon1, const GeometryPolygon_t* polygon2) {
	...
	for (i = 0; i < polygon1->v_indices_cnt; ) {
		...
		if (Segment_Intersect_Polygon((const CCTNum_t(*)[3])edge, polygon2)) {
			return 1;
		}
	}
	for (i = 0; i < polygon2->v_indices_cnt; ) {
		...
		if (Segment_Intersect_Polygon((const CCTNum_t(*)[3])edge, polygon1)) {
			return 1;
		}
	}
	return 0;
...
int ConvexMesh_Intersect_Polygon(const GeometryMesh_t* mesh, const GeometryPolygon_t* polygon, int* ret_all_one_side) {
	int flag_sign = 0, all_one_side = 1;
	unsigned int i;
	for (i = 0; i < mesh->v_indices_cnt; ++i) {
		const CCTNum_t* mesh_p = mesh->v[mesh->v_indices[i]];
	...
	for (i = 0; i < mesh->edge_indices_cnt; ) {
		...
		if (Segment_Intersect_Polygon((const CCTNum_t(*)[3])edge, polygon)) {
			return 1;
		}
	}
	for (i = 0; i < polygon->v_indices_cnt; ) {
...
int mathGeometryIntersect(const GeometryBodyRef_t* one, const GeometryBodyRef_t* two) {
	...
	if (GEOMETRY_BODY_POINT == one->type) {
		...
		switch (two->type) {
			...
			case GEOMETRY_BODY_POLYGON:
			{
				return Segment_Intersect_Polygon(one_segment_v, two->polygon);
			}
			case GEOMETRY_BODY_CONVEX_MESH:
			{
		...
		switch (two->type) {
			...
			case GEOMETRY_BODY_SEGMENT:
			{
				return Segment_Intersect_Polygon((const CCTNum_t(*)[3])two->segment->v, one->polygon);
			}
			case GEOMETRY_BODY_PLANE:
			{
				const GeometryPlane_t* plane = two->plane;
